...
static int imx296_read(struct camera_common_data *s_data, u32 addr)
{
	...
	ret = regmap_raw_read(s_data->regmap, addr & IMX296_REG_ADDR_MASK, data,
...
static int imx296_write(struct camera_common_data *s_data, u32 addr, u32 value, int *err)
{
...
static int imx296_set_group_hold(struct tegracam_device *tc_dev, bool val)
{
	...
	err = imx296_write_reg(s_data, IMX296_GROUP_HOLD_ADDR, val);
	if (err) {
...
static int imx296_set_gain(struct tegracam_device *tc_dev, s64 val)
{
	...
	imx296_write(sensor, IMX296_GAIN, gain, &err);
...
static int imx296_set_frame_rate(struct tegracam_device *tc_dev, s64 val)
{
	...
	imx296_write(sensor, IMX296_VMAX, vmax, &err);
        if (err == 0) {
		imx296_write(sensor, IMX296_HMAX, 1100, &err);
		priv->vmax = vmax;
	}
...
static int imx296_set_mode(struct tegracam_device *tc_dev)
{
	...
	imx296_write(sensor, IMX296_FID0_ROI, 0, &err);
	imx296_write(sensor, IMX296_MIPIC_AREA3W, IMX296_PIXEL_ARRAY_HEIGHT,
		     &err);
	 ...
	 */
	unsigned int vmax = priv->vmax;

	imx296_write(sensor, IMX296_HMAX, 1100, &err);
	imx296_write(sensor, IMX296_VMAX, priv->vmax, &err);

	for (i = 0; i < ARRAY_SIZE(sensor->clk_params->incksel); ++i)
		imx296_write(sensor, IMX296_INCKSEL(i),
			     sensor->clk_params->incksel[i], &err);
	imx296_write(sensor, IMX296_GTTABLENUM, 0xc5, &err);
	imx296_write(sensor, IMX296_CTRL418C, sensor->clk_params->ctrl418c,
		     &err);

	imx296_write(sensor, IMX296_GAINDLY, IMX296_GAINDLY_1FRAME, &err);
	imx296_write(sensor, IMX296_BLKLEVEL, 0x03c, &err);
...
static int imx296_board_setup(struct imx296 *priv)
{
	...
	err = imx296_identify_model(sensor);
	if (err < 0)
		goto err_power;

	/* Probe FRM_CNT register */
	err = imx296_read_reg(s_data, 0x05, &reg_val);
	if (err) {
